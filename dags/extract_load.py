...
sql_load_dimensions_queries = {
    ...
    """,
    " d_customer ": """ TRUNCATE TABLE dw.d_customer CASCADE;
...
def _transfer_data(**context):
    with connect(
        host="source_db", database="postgresDB", user="dbadmin", password="dbadmin123"
    ) as source_conn, connect(
        host="destiny_db", database="postgresDB", user="dbadmin", password="dbadmin123"
    ) as dest_conn:
...
with DAG(
    ...
    transfer_task = PythonOperator(
        task_id="transfer_data", python_callable=_transfer_data, dag=dag
    )
